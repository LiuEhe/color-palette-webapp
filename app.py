...
def generate_random_colors():
    ...
    for _ in range(5):
        ...
        r = np.random.randint(0,255)     #TODO
        g = np.random.randint(0,255)        #TODO
        b = np.random.randint(0,255)        #TODO
...
def upload_image():
    ...
    image = cv2.imread(filepath)        #TODO # 读取图像
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)        #TODO # BGR->RGB
    ...
    if crop_box_coords:
        ...
        image =  image[x:x+width,y:y+height]       #TODO # 从原image中截取左上角是(x,y)，宽度为width，高度为height的区域
